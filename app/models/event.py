...
class Event(db.Model):
    ...
    game = Column(String(64), default="valorant", nullable=False)
    format = Column(String(64), default="single_elimination", nullable=False)
    prize_pool = Column(Numeric(12,2), default=0, nullable=False)
    team_size = Column(Integer, default=5, nullable=False)
    match_rules = Column(Text)
    region = Column(String(80))
    server = Column(String(80))
    check_in_starts_at = Column(DateTime(timezone=True))
    check_in_ends_at = Column(DateTime(timezone=True))
